...
def read_submenu(
    ...
    if db_submenu is None:
        raise HTTPException(HTTPStatus.NOT_FOUND, detail="submenu not found")
    db_submenu.dishes_count = (
        crud.get_dishes_count_in_submenu(db, submenu_id=submenu_id)
    )
    return db_submenu
...
def read_all_submenus(menu_id: UUID, db: Session = Depends(get_db)):
    submenus = crud.get_all_submenus(db, menu_id=menu_id)
    for row in submenus:
        row.dishes_count = crud.get_dishes_count_in_submenu(
            db, submenu_id=row.id
        )
    return submenus
...
def create_submenu(
    ...
    db_submenu.dishes_count = (
        crud.get_dishes_count_in_submenu(db, submenu_id=db_submenu.id)
    )
    return db_submenu
...
def update_submenu(
    ...
    db_submenu.dishes_count = crud.get_dishes_count_in_submenu(
        db,
        submenu_id=submenu_id
    )
    return db_submenu
