...
def read_menu(menu_id: UUID, db: Session = Depends(get_db)):
    ...
    db_menu.submenus_count = (
        crud.get_submenu_count_in_menu(db, menu_id=menu_id)
    )
    db_menu.dishes_count = crud.get_dishes_count_in_menu(db, menu_id=menu_id)
    return db_menu
...
def read_menus(db: Session = Depends(get_db)):
    ...
    for row in menus:
        row.submenus_count = (
            crud.get_submenu_count_in_menu(db, menu_id=row.id)
        )
        row.dishes_count = crud.get_dishes_count_in_menu(db, menu_id=row.id)
    return menus
...
def create_menu(menu: schemas.CreateMenu, db: Session = Depends(get_db)):
    ...
    db_menu.submenus_count = (
        crud.get_submenu_count_in_menu(db, menu_id=db_menu.id)
    )
    db_menu.dishes_count = crud.get_dishes_count_in_menu(
...
def update_menu(
    menu_id: UUID, menu: schemas.CreateMenu, db: Session = Depends(get_db)
):
    db_menu = crud.get_menu(db, menu_id=menu_id)
    if db_menu is None:
        raise HTTPException(
